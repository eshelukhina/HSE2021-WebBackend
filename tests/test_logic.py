...
class MyTestCase(unittest.TestCase):
    ...
    def test_is_valid_name(self):
        for x in range(0, 1000):
            ...
            if number_of_words == 2:
                self.assertEqual(is_valid_name(rnd_str), True)
            else:
                self.assertEqual(is_valid_name(rnd_str), False)
    ...
    def test_is_valid_age(self):
        for x in range(0, 1000):
            ...
            if num >= 0:
                self.assertEqual(is_valid_age(num), True)
            else:
                self.assertEqual(is_valid_age(num), False)
